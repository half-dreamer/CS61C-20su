...
main:
    ...
    #print the list
    add a0, s0, x0  
    jal ra, print_list # pass in a0(argument),the head of node list

    # print a newline
    jal ra, print_newline # don't need argument to pass in 
...
recursion: beq a0, x0, done    # If we were given a null pointer (address 0), we're done.
    ...
    ### YOUR CODE HERE ###
    jalr ra, a1, 0 # it 's better to use jalr or jal ,because psuedo jump instructions like j or jr don't store return address in ra
                    # so we need to manually store it
    ...
    # recurse
    ### YOUR CODE HERE ###
    jal ra recursion  # actually I think j map is also correct, because we don't need ra to return 
...
create_default_list:
    addi sp, sp, -12
    sw  ra, 0(sp)   # actually, ra should be stored by caller,but we use jal ,so there is no way to store ra in caller
    sw  s0, 4(sp)
    sw  s1, 8(sp)
...
# the print function is kind of like mutual recursion (or say , the print need a 
# helper function to execute recursion)
print_list:
    bne a0, x0, printMeAndRecurse   # a0 is the head of list
...
printMeAndRecurse:
    ...
    ecall
    addi    a1, x0, ' '     # a0 gets address of string containing space (I guess the value of ' ' is the address of String ,like C )
    addi    a0, x0, 11      # prepare for print string syscall (print ' ')
    ecall
